...
engine.on('game_starting', function (info) {
    ...
    if (currentBet <= engine.getBalance()) { // Ensure we have enough to bet
        if (currentBet > (maximumBet * 100)) { // Ensure you only bet the maximum.
            console.warn('%c [Warn] Bet size exceeds maximum bet, lowering bet to ' + (maximumBet * 100) + ' bits', 'background: #FE2E2E');
        ...
        if (engine.getBalance() < 100) {
            console.error('[Bot] Insufficent funds to do anything... stopping', 'background: #FE2E2E');
            engine.stop();
        }
        else {
            console.warn('%c [Bot] Insufficent funds to bet ' + (currentBet / 100) + ' bits.', 'background: #FE2E2E');
            console.warn('%c [Bot] Resetting to 1 bit basebet', 'background: #FE2E2E');
            baseBet = 1;
            baseSatoshi = 100;
        }
    }
...
engine.on('game_started', function (data) {
    if (!firstGame) {
        ...
        if (infoCount > 5) {
            if (lastLossCount != 0) {
                console.log('%c [Info] last lost was ' + (lastLossCount + 1) + ' games ago', 'background: #fdff00');
            }
            else {
                console.log('%c [Info] Still winning!', 'background: #fdff00');
            }
            infoCount = 0;
        }
...
engine.on('cashed_out', function (data) {
    if (data.username == engine.getUsername()) {
        console.log('%c [Bot] Successfully cashed out at ' + (data.stopped_at / 100) + 'x', 'background: #04B431');
        didWin = true;
    }
});
...
engine.on('game_crash', function (data) {
    if (!firstGame) {
        ...
        if (!didWin) {
            console.log('%c [Bot] Lost ' + (currentBet / 100), 'background: #FE2E2E');
        }
    }
});